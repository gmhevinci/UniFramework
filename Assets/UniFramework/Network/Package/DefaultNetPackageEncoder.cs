...
namespace UniFramework.Network
{
	...
	public class DefaultNetPackageEncoder : INetPackageEncoder
	{
		...
		public void Encode(int packageBodyMaxSize, RingBuffer ringBuffer, INetPackage encodePackage)
		{
			...
			if (package == null)
			{
				_handleErrorCallback(false, $"The package object is invalid : {encodePackage.GetType()}");
				return;
			}
			...
			{
				_handleErrorCallback(false, $"The package BodyBytes field is null : {encodePackage.GetType()}");
				return;
			}
			...
			{
				_handleErrorCallback(false, $"The package {package.MsgID} body size is larger than NetworkDefine.PackageBodyMaxSize");
				return;
			}

			// 写入长度
			int packetSize = PackageHeaderIDFiledByte + bodyData.Length;	
			ringBuffer.WriteInt(packetSize);
